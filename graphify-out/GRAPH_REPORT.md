...
## Corpus Check
- 115 files · ~470,709 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 949 nodes · 1201 edges · 83 communities (72 shown, 11 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 85 edges (avg confidence: 0.79)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `59860a85`
...
4. `deterministic_gzip_csv()` - 15 edges
5. `build()` - 13 edges
...
## Knowledge Gaps
- **484 isolated node(s):** `run_pipeline.sh script`, `build.sh script`, `Mandatory Startup Context Recovery`, `Graphify`, `Project Layout` (+479 more)
...
- **Are the 11 inferred relationships involving `deterministic_gzip_csv()` (e.g. with `analyze()` and `audit_member()`) actually correct?**
  _`deterministic_gzip_csv()` has 11 INFERRED edges - model-reasoned connections that need verification._
