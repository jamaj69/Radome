...
def build(selection, terrain_root, output, size=80, step=8):
    sites=[]
...
if __name__=="__main__":
    p=argparse.ArgumentParser();p.add_argument("--selection",type=Path,required=True);p.add_argument("--terrain-root",type=Path,required=True);p.add_argument("--output",type=Path,required=True);a=p.parse_args();build(a.selection,a.terrain_root,a.output)
